...
سیاست فعلی `external_access.credential_free_only: true` است: ناظر نیازمند حساب یا API key اجازهٔ درخواست ندارد؛ حتی اگر در `reviewers.local.json` فعال شده باشد. adapterهای قبلی برای سازگاری حفظ شده‌اند، اما تحت این سیاست قابل استفاده نیستند. فعلاً ناظر خارجیِ بدون حساب و کلید که اتصال واقعی آن تأیید شده باشد نداریم؛ Candidate در انتظار ناظر می‌ماند و تأیید انسانی یا یادگیری را دور نمی‌زند. Free Tier به معنی بدون حساب و کلید نیست. توضیحات کلیدها در ادامه مربوط به حالت اختیاری قدیمی است، نه پیش‌نیاز اجرای محلی یا تنظیم فعلی.

خواندن منابع عمومی وب در مسیر موجود بدون ارسال کلید و کوکی انجام می‌شود، اما منبع دانش جای ناظر را نمی‌گیرد. شبکه، محدودیت سایت و در دسترس بودن منبع همچنان لازم‌اند. بررسی واقعی منبع عمومی در محیط اجرای فعلی با خطای DNS متوقف شد؛ اتصال زنده تأیید نشده است.
...
بررسی شواهد پاسخ، وضعیت پشتیبانی، نفی، کمبود شواهد و تعارض را در trace ثبت می‌کند.
برای ویژگی‌های تک‌مقداری، دو ادعای متناقض تا زمان اصلاح صریح پاسخ قطعی تولید نمی‌کنند؛
جدیدتر بودن یک ادعا به‌تنهایی به معنی درست‌تر بودن آن نیست. این بررسی محلی و محدود
به روابط ساختاریافته است و اثبات حقیقت عمومی محسوب نمی‌شود.


اجرای `sandbox_python` فقط زیرمجموعهٔ محدودی از پایتون را می‌پذیرد و به محدودیت‌های
منابع سیستم‌عامل نیاز دارد. روی Windows بومی، تا زمان وجود اجرای ایزولهٔ پشتیبانی‌شده،
این ابزار با دلیل `sandbox_resource_limits_unavailable` اجرا نمی‌شود؛ چت و بازبینی
یادگیری از آن مستقل‌اند. این ابزار جایگزین container برای اجرای کد دلخواه نیست.


آزمایش موفق sandbox به‌تنهایی اثبات یادگیری موضوع نیست. آزمایش‌های عمومی با وضعیت
`diagnostic_only` ثبت می‌شوند. مسیر فعلی ارتقای قابلیت فقط قرارداد اجرایی جمع اعداد
صحیح را همراه کد و آزمون ورودی‌های جدید نگه می‌دارد؛ ادعای منبع را تأییدشده نمی‌نامد
و بابت این آزمایش‌ها XP ایجاد نمی‌کند. ارتقای مهارت همچنان نیازمند دو تأیید است.
