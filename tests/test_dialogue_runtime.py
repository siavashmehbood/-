...
class RuntimeDialogueTests(unittest.TestCase):
    def setUp(self):
        ...
        c['memory']['db']='data/test.db'; c['runtime']['event_log']='logs/test.jsonl'
        (self.tmp/'config.json').write_text(json.dumps(c,ensure_ascii=False),encoding='utf-8')
        self.r=IranRuntime(self.tmp)
    def tearDown(self):
        self.r.close(); shutil.rmtree(self.tmp,ignore_errors=True)
    def ask(self,text): return self.r.handle(text)
    ...
    def test_multi_intent(self):
        ...
        self.assertIn('۱)',a); self.assertIn('۲)',a); self.assertIn('۳)',a)
    def test_topic_restore(self):
        self.ask('پایتون چیه؟'); self.ask('حافظه چیه؟'); a=self.ask('موضوع قبلی رو ادامه بده.')
        self.assertIn('پایتون',a)
    def test_persistent_state(self):
        self.ask('پایتون چیه؟'); self.r.close()
        r2=IranRuntime(self.tmp); self.assertEqual(r2.conversation_snapshot()['current_topic'],'پایتون چیه؟'); r2.close()
