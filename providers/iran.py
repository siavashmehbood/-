...
def _generate_v3(self, messages, **kwargs):
    ...
    if 'سطحی' in low and ('چرا' in low or 'پیشرفت' in low):
        ...
        return answer
    if 'مستقل' in low and ('مدل خارجی' in low or 'بدون مدل' in low):
        return ('بله. دلیل این تصمیم این بود که «ایران» قرار است هسته مستقل خودش را داشته باشد و پاسخ‌ها را با منطق نمادین محلی تولید کند. '
