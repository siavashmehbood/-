...
def _bridge_conversation_response(self, text, parsed, cycle, history, frame):
    t = str(text).strip().replace('ي','ی').replace('ك','ک'); low=t.lower(); frame=frame or {}; rows=[str(x) for x in (history or [])]
    topic=str(frame.get('topic') or '')
    if not topic or len(topic)<4 or topic.rstrip('؟?') in {'چرا','خب','چطور','بیشتر بگو','ادامه بده'}:
        for item in reversed(rows):
            if item.strip() and item.strip()!=t and any(k in item for k in ('پایتون','حافظه','پروژه','Django')):
    ...
    if 'برای پروژه من' in t and 'پایتون' in topic.lower(): return 'بله، برای پروژه تو پایتون می‌تواند انتخاب خوبی باشد؛ مخصوصاً برای منطق برنامه، API، اتوماسیون و پردازش داده. انتخاب نهایی به معماری و محدودیت‌های خود پروژه بستگی دارد.'
...
if not hasattr(LocalResponseEngine,'_iran_conversation_bridge_base'): LocalResponseEngine._iran_conversation_bridge_base=LocalResponseEngine.respond
...
class Agent:
    def __init__(self,brain,memory,max_history=16,conversation_path=None):
        ...
        if conversation_path is None:
            try:
                db_path=self.memory.conn.execute('PRAGMA database_list').fetchone()[2]; conversation_path=str(Path(db_path).with_name('conversation_state.json')) if db_path else None
            except Exception: conversation_path=None
        self.conversation=ConversationState(state_path=str(conversation_path or 'data/conversation_state.json')); self.conversation_intelligence=ConversationIntelligence(self.conversation)
    def build_messages(self,user_text):
        provider=getattr(self.brain,'provider',None)
        if provider is not None and hasattr(provider,'frame'): provider.frame.update({'conversation_topic':self.conversation.current_topic,'conversation_goal':self.conversation.active_goal,'conversation_referent':self.conversation.referent,'conversation_correction':self.conversation.correction,'conversation_confidence':self.conversation.conversation_confidence})
        memories=self.memory.working_context(user_text,self.max_history); context='\n'.join(f'[{k}] {c}' for k,c,_ in memories); messages=[{'role':'system','content':SYSTEM_PROMPT}]
    ...
    def respond(self,user_text):
        context=self.conversation_intelligence.build_context(user_text,self.conversation.turns); context=self.conversation_intelligence.enrich(context,self.memory); answer=self.brain.ask(self.build_messages(user_text),cognitive_context=context); answer,verification=self.conversation_intelligence.validate_and_repair(context,answer)
        ...
        self.memory.add('user',user_text,.7); self.memory.add('assistant',answer,.6); self.memory.add('conversation_verification',str({'status':verification.status,'score':verification.score,'reasons':verification.reasons}),.5); self.memory.add('event','response generated at '+datetime.now().isoformat(timespec='seconds'),.2); return answer
_old_respond=Agent.respond
Agent.respond=lambda self,user_text: _old_respond(self,user_text)
