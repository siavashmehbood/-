...
class ConversationState:
    ...
    state_path: str = 'data/conversation_state.json'
    ...
    @property
    def topic(self): return self.current_topic
    @topic.setter
    def topic(self, value): self.current_topic = clean(value)
    @property
    def goal(self): return self.active_goal
    @goal.setter
    def goal(self, value): self.active_goal = clean(value)
    @property
    def last_user(self): return self.last_user_message
    @last_user.setter
    def last_user(self, value): self.last_user_message = clean(value)
    @property
    def last_assistant(self): return self.last_assistant_answer
    @last_assistant.setter
    def last_assistant(self, value): self.last_assistant_answer = clean(value)
    @property
    def referent(self): return self.references.get('current', '')
    @referent.setter
    def referent(self, value):
        value = clean(value)
        if value: self.references['current'] = value
    @property
    def correction(self): return self.corrections[-1] if self.corrections else ''
    ...
    def update(self, user_text, assistant_text='', parsed=None, answer_type=''):
        ...
        if any(x in user_text for x in ('؟', '?', 'چرا', 'چطور', 'چگونه', 'آیا')):
            self.current_question = user_text
        ...
        for item in parsed.get('entities') or []:
            ...
            if substantive(value) and value not in self.entities: self.entities.append(value)
        if self._is_correction(user_text): self._record_correction(user_text)
        reference = self.resolve_reference(user_text)
        if reference: self.referent = reference
        elif not self._is_follow_up(user_text) and not self._is_correction(user_text) and substantive(user_text): self.set_topic(user_text)
        if self._is_follow_up(user_text) and not self.current_topic: self.set_topic(self.last_user_message)
        if assistant_text:
        ...
        if assistant_text: self.turns.append(asdict(Turn('assistant', assistant_text, self.current_topic, self.active_goal)))
        self.turns = self.turns[-80:]
        if previous and self.current_topic and previous != self.current_topic and substantive(previous):
            if previous not in self.topic_stack: self.topic_stack.append(previous)
    ...
    def set_topic(self, topic):
        ...
        if self.current_topic and self.current_topic != topic and substantive(self.current_topic):
            self.topic_stack.append(self.current_topic); self.topic_stack = self.topic_stack[-20:]
        self.current_topic = topic; self.references['current'] = topic

    def previous_topic(self): return self.topic_stack[-1] if self.topic_stack else ''

    def restore_previous_topic(self):
        if not self.topic_stack: return self.current_topic
        previous = self.topic_stack.pop()
        if self.current_topic and self.current_topic != previous: self.topic_stack.append(self.current_topic)
        self.current_topic = previous; self.references['current'] = previous; self.save(); return previous

    def resolve_reference(self, text):
        text = clean(text)
        if 'موضوع قبلی' in text or 'برگردیم' in text: return self.restore_previous_topic()
        if 'همون بحث اول' in text and self.topic_stack: return self.topic_stack[0]
        if not any(w in text for w in REF_WORDS): return ''
        candidate = self.current_topic or self.active_goal or self.last_user_message
        if candidate:
            for word in REF_WORDS:
                if word in text: self.references[word] = candidate
        return candidate
    ...
    def _record_correction(self, text):
        ...
        if substantive(target) and target != text:
            self.set_topic(target); self.references['correction'] = target
    ...
    def _is_correction(self, text): return any(clean(text).startswith(x) for x in CORRECTION_WORDS)
    ...
    def snapshot(self):
        data = asdict(self); data.pop('state_path', None); return data
    def save(self, path=None):
        path = path or self.state_path
        if not path: return
        self.state_path = str(path); p = Path(path); p.parent.mkdir(parents=True, exist_ok=True)
    ...
    def load(self, path=None):
        ...
        try:
            ...
            for key, value in data.items():
                if key != 'state_path' and hasattr(self, key): setattr(self, key, value)
