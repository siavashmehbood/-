...
class LocalResponseEngine:
    ...
    def direct_fact(self, text, parsed=None):
        ...
        facts=(
            ...
            (('پایتخت فرانسه','پایتخت فرانسه کجاست'), 'پایتخت فرانسه پاریس است.', 'high'),
            (('تعداد روزهای هفته','هفته چند روز'), 'هفته هفت روز دارد.', 'high'),
            (('آب در چند درجه می‌جوشد','دمای جوش آب','آب چند درجه می‌جوشد'), 'آب در فشار استاندارد سطح دریا در ۱۰۰ درجه سلسیوس می‌جوشد.', 'high'),
...
def _why_v2(self,text,cycle,history):
    ...
    if any(x in low for x in ('سطحی','جواب','پاسخ','هوشمند','درک زبان')):
        ...
        parts.append('علت کندی فقط یک حدس نیست: چرخه شناختی قبلاً دوبار در هر نوبت اجرا می‌شد و این باعث محاسبه و ثبت تکراری می‌شد؛ این مسیر اصلاح شده است.')
