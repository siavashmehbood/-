...
class QuestionAnalyzer:
    def analyze(self, text: str) -> tuple[str, list[str]]:
        ...
        if any(x in t for x in ('چطور', 'چگونه', 'چه جوری')):
            return 'how', units
        if 'آیا' in t or t.endswith('?') or t.endswith('؟'):
            return 'question', units
        if any(x in t for x in ('مقایسه', 'فرق', 'تفاوت', 'بهتره')):
            return 'compare', units
        if any(x in t for x in ('ادامه بده', 'بیشتر بگو', 'توضیح بده', 'ساده‌تر', 'کوتاه‌تر', 'بهترش کن')):
            return 'follow_up', units
...
class AnswerPlanner:
    def build(self, context: CognitiveContext) -> list[str]:
        if context.question_type == 'multi_intent':
            return [f'answer: {u}' for u in context.question_units]
        if context.question_type in {'question', 'why', 'how', 'compare'}:
            return ['direct_answer', 'ground_with_local_evidence', 'state_uncertainty_if_needed']
        if context.question_type == 'follow_up':
            return ['resolve_reference', 'continue_previous_topic', 'answer_directly']
...
class AnswerVerifier:
    def verify(self, context: CognitiveContext, answer: str) -> AnswerVerification:
        ...
        if not answer:
            return AnswerVerification('REPAIR', ['empty_answer'], score=0.0)
        if context.references and not any(v[:20] in answer for v in context.references.values()):
            if context.question_type == 'follow_up':
                return AnswerVerification('REPAIR', ['wrong_reference'], score=0.25)
        missing = []
        if context.question_type == 'multi_intent':
            for unit in context.question_units:
                key = re.sub(r'[^آ-یA-Za-z0-9 ]', ' ', unit).strip()[:18]
                if key and not any(tok in answer for tok in key.split()[:2]):
                    missing.append(unit)
        generic = any(x in answer for x in ('برای پاسخ به این پرسش', 'چند عامل قابل بررسی', 'اطلاعات بیشتری'))
        if missing:
            return AnswerVerification('REPAIR', ['incomplete_answer'], missing, score=0.35)
        if generic and len(context.user_message.split()) > 2:
            return AnswerVerification('REPAIR', ['too_generic'], score=0.4)
...
class AnswerRepair:
    def repair(self, context: CognitiveContext, answer: str, verification: AnswerVerification) -> str:
        ...
        if 'incomplete_answer' in verification.reasons:
            parts = [answer]
            for unit in verification.missing_units:
                parts.append(f'بخش «{unit}» هم باید جداگانه بررسی شود.')
            return '\n'.join(parts)
        if 'too_generic' in verification.reasons:
            return f'پاسخ کوتاه: {answer}'
...
class ConversationIntelligence:
    ...
    def build_context(self, text: str, history=None) -> CognitiveContext:
        qtype, units = self.analyzer.analyze(text)
        refs = self.resolver.resolve(text, self.state)
        history = history or []
    ...
    def enrich(self, context: CognitiveContext, memory=None):
        if memory is not None:
            try:
                rows = memory.working_context(context.user_message, 8)
                context.relevant_memory = [str(c) for _, c, _ in rows]
            except Exception:
                pass
    ...
    def validate_and_repair(self, context: CognitiveContext, answer: str) -> tuple[str, AnswerVerification]:
        ...
        if verification.status != 'PASS':
            answer = self.repair.repair(context, answer, verification)
            verification = self.verifier.verify(context, answer)
